# گزارش آزمایش
همانطور که در دستور کار خواسته شده است؛ در این آزمایش هر commit معادل با یک یا چند refactor است که در پیام آن، نوع بازآرایی مشخص شده است. در ادامه، انواع مختلف refactor هایی که در این آزمایش انجام شده‌اند، بررسی خواهند می‌شوند.

### Replace Conditional with Polymorphism
در این سناریو، یک switch statement در کد وجود دارد که می‌تواند با ساختاری شامل یک کلاس پایه و تعدادی کلاس ارث‌برنده از آن کلاس پایه جایگزین شود. تصویری که از این سناریو که در کد وجود دارد؛ به صورت زیر آورده شده است.
...
در اینجا، برای متد toString چندین پیاده‌سازی مختلف فرض شده است که این انواع با caseهای متعدد handle شده‌اند. حال قرار است این تکه از کد بازآرایی شود. مطابق تصویر زیر، تعدادی کلاس ایجاد شده‌اند که همگی از کلاس پایه Address ارث‌بری می‌کنند. در این کلاس‌ها تابع toString مخصوص خودشان پیاده‌سازی شده است. بدین شکل، چندریختی جایگزین ساختار شرطی اولیه شده است. تغییرات کامل در یک commit با نام بازآرایی گفته شده، موجود است.
...
### Facade in Parser
این الگو زمانی کارآمد است که دو زیرسیستم وجود داشته باشد و عناصر داخلی این دو زیر سیستم به هم وابستگی داشته و یا اصطلاحا coupled باشند. در این سناریو، به ازای هر زیرسیستم، یک واسط ایجاد می‌شود و پس از آن، تمام ارتباطات بین اجزای این دو زیرسیستم فقط از طریق این دو واسط صورت می‌پذیرد. در مثالی که در آزمایش وجود دارد، برای زیرسیستم Parser یک واسط به صورت زیر ایجاد و استفاده شده است. یعنی کلاس Main به جای آنکه مستقیما با کلاس‌های بسته Parser ارتباط برقرار کند؛ با واسط آن ارتباط برقرار می‌نماید.
...
### Facade in Symbol
مورد دومی که از بازآرایی facade در این آزمایش وجود دارد، این است که در کلاس CodeGenerator ارتباط‌های زیادی با کلاس‌های concrete در بسته Symbol وجود دارد. به همین علت، برای بسته Symbol مطابق شکل زیر، یک واسط SymbolFacade ایجاد شده است که پس از این تمام ارتباط‌های این دو کلاس از طریق این واسط انجام پذیرد.
...
در ادامه قسمتی از پیاده‌سازی و توابع موجود در واسط SymbolFacade آورده شده است تا تمام ارتباطات میان این کلاس و گلاس‌های دیگر پوشش داده شود.
...
### Separate Query from Modifier
 برای درک این نمونه از بازآرایی، فرض کنید تابعی وجود دارد که یک مقدار از یک object را درون خود تغییر می‌دهد ولی در کنار این کار، یک مقدار دیگری را هم return می‌کند. می‌توان این تابع را در دو تابع دیگر بازآرایی کرد. به این صورت که یک تابع، کار query یا get کردن را انجام می‌دهد و تابع دیگر وظیفه modify کردن را بر عهده دارد. در مثال این آزمایش، یک تابع کلی defMain وجود دارد که در ۲ تابع queryMain و modifyMain بازآرایی شده است. تابع اول، کار بازیابی query و بازگرداندن مقدار خواسته شده را انجام می‌دهد و تابع دوم، اضافه نمودن به جدول و modify کردن را بر عهده دارد.
...
### Fix DIP
همانطور که در ویدیوی آموزشی گفته شده است؛ به جای استفاده از عبارت 
```
...
```
استفاده شود تا DIP نقض نشود. کلاس ArrayList که یک کلاس concrete است؛ واسط List را پیاده‌سازی می‌کند. اگر از مقداردهی اول استفاده شود؛ اعمال هر گونه تغییرات در arraylist می‌تواند موجب از کار افتادن کد و به خطا افتادن کاربر شود. اما استفاده از واسط List مانع از انتشار تغییرات شده و هر گونه دگرگونی در کلاس‌های فرزند، در پشت آن متوقف می‌شود و کاربر متوجه آن نخواهد شد. تصویر این تغییرات به شکل زیر است.
...
در ادامه، تعدادی بازآرایی شامل حذف کدهای استفاده نشده، تغییر نام‌گذاری کلاس‌ها و متدها، تصحیح typoها و ... انجام شده است که در commit های مشخصی با همین نام‌ها وجود دارند و به علت اختصار از ذکر آنها صرف نظر می‌شود.
